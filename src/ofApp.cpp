...
void ofApp::update(){
    ...
    float t = (2 + ofGetElapsedTimef()) * 0.4;
    ...
    if (move) {
        for (int i = 0; i < particles.size(); i++) {
            particles[i].addAttractionForce(current.x, current.y, current.z, 10000, 1.1);
            particles[i].update();
        ...
        for (int i = 0; i < 100; i++) {
            ParticleVec3 p;
            p.position = ofVec3f(current.x, current.y, current.z);
            float length = ofRandom(2.0);
            float angle = ofRandom(100.0);
